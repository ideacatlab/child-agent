...
isolation, point ``SCION_SANDBOX_DOCKER_IMAGE`` at an image and commands route
...
def run_command(
    ...
    *,
    ...
    Honors ``SCION_SANDBOX_DOCKER_IMAGE`` to run inside a container.
    """
    docker_image = os.environ.get("SCION_SANDBOX_DOCKER_IMAGE", "").strip()
    run_env = {**os.environ, **(env or {})}
    workdir = str(cwd or Path.cwd())

    if docker_image:
        argv = [
            "docker", "run", "--rm", "--network", os.environ.get("SCION_SANDBOX_NET", "none"),
...
def run_python_snippet(code: str, *, cwd: Path | None = None, timeout: int = 60) -> tuple[int, str]:
    ...
    Honors ``SCION_SANDBOX_DOCKER_IMAGE`` (routes through the same container path
    as :func:`run_command`) so shell and Python execution share one boundary.
    """
    if os.environ.get("SCION_SANDBOX_DOCKER_IMAGE", "").strip():
        return run_command(
            "python3 - <<'__SCION_PY__'\n" + code + "\n__SCION_PY__",
            cwd=cwd,
            timeout=timeout,
        )
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            ...
            for dec in node.decorator_list:
                ...
                if dname in ("tool", "scion.tools.tool", "base.tool"):
