...
queue, and acks "queued — working on it" (ali-fleet-recovery's sentinel-bot: the
deterministic layer never blocks on the brain). Claude Code drains the queue and
replies with ``scion tg send <chat_id> "…"``.
...
def send(chat_id: str | int, text: str, settings: Settings | None = None) -> bool:
    """Send a message (chunked at 4096). Used by ``scion tg send``."""
...
class TelegramReceiver:
    ...
    def _handle(self, msg: dict | None) -> None:
        ...
        ack = (
            f"📥 Queued *#{task_id}* — {self.s.agent_name} will work this and reply here."
            if is_new
    ...
    def _command(self, text: str, chat_id: int) -> None:
        ...
        if cmd in ("start", "help"):
            self.client.send_message(
                f"*{self.s.agent_name}* is online. Send me anything — it goes on the work "
